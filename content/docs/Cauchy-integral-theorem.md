...
<p>
  ...
  $\alpha, \beta\in\CC$を結ぶ任意の曲線$C_1, C_2$に対して
  $$
...
{{< hint >}}
  ...
  <p>
    ...
    \begin{align*}
      ...
      &=\int_{\partial R_n}(f(z)-f(z_*)-(z-z_*)f'(z_*))dz
    \end{align*}
    が成り立つ。
    上の不等式により
    \begin{align*}
...
{{< hint >}}
  空でない連結開集合$\Omega\subset\CC$を領域という。
{{< /hint >}}
...
{{< hint >}}
  閉曲線$C$の$z=a$についての\textbf{回転数}$n(C,a)$を
  \begin{align*}
...
{{< hint >}}
  領域$D$内のサイクル$C$が、$D$に関して\textbf{$0$にホモローグ}であるとは、
  $D$の補集合の任意の点$a$に対し、$n(C,a)=0$であること。
{{< /hint >}}

{{< hint >}}
  $D=\CC$とし、$C$を反時計回りの単位円周とするとこれは$D$に関して$0$にホモローグ。
  一方で$D=\CC\setminus\{0\}$とすると、この$C$は$D$に関して$0$にホモローグではない。
{{< /hint >}}
...
{{< hint >}}
  $R\subset\CC$を長方形領域とし、$f$は$R$の境界および内部で正則であるとする。
  このとき、
  $$
  \int_{\partial R}f(z)dz=0
  $$
